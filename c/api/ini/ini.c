...
#define INI_READ_ARRAY(type, is_unsigned) do \
{ \
	char *s = NULL;\
	int ret = ini_read_str(handler, section, name, &s, NULL);\
	if (!ret) {\
		...
		do {\
			if (is_unsigned) {\
				array[array_len++] = (type) strtoull(p, NULL, 0);\
			} else {\
				array[array_len++] = (type) strtoul(p, NULL, 0);\
			}\
		} while(is_array && (p = strtok(NULL, delim)) && (array_len < (int)array_size));\
		free(s);\
		return array_len;\
	} else if (ret > 0) {\
